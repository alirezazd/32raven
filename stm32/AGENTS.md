...
- All NVIC priorities flow through `stm32/Drivers/USER/inc/irq_priority.hpp`. Don't hardcode literals at `NVIC_SetPriority` callsites — add a `constexpr` (in the right lane) and reference it. The header's top comment documents the ordering invariant.
...
- Every PCB pin assignment lives in `PINMAP_ENTRIES` in `scripts/generate_stm32_config.py` and is emitted into the autogenerated `stm32/Drivers/USER/inc/stm32_config.hpp` as `inline const BoardPin board::k<Name> = {...};`. There is no hand-edited `board.hpp` — adding a pin always means editing the generator + Kconfig, never a header.
