...
PATTERNS = {
    ...
    "rtc_ram": r"^\s*rtc_iram_seg\s+0x[0-9a-fA-F]+"
    r"\s+0x([0-9a-fA-F]+)\s+\S+\s*$",
    "rtc_reserved": r"^\s*rtc_reserved_seg\s+0x[0-9a-fA-F]+"
    r"\s+0x([0-9a-fA-F]+)\s+\S+\s*$",
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description="Print ESP32 RAM, flash-app, and RTC usage."
    )
    ...
    if len(maps) != 1:
        raise SystemExit(f"expected exactly one .map in {build}")
    map_text = maps[0].read_text(encoding="utf-8", errors="replace")

    def get(key):
        m = re.search(PATTERNS[key], map_text, re.MULTILINE)
        return int(m.group(1), 16)

    dram = (get("iram") + get("data") + get("bss"), get("diram"))
    rtc = (
        get("rtc_slow") + get("rtc_fast") + get("rtc_used_reserved"),
        get("rtc_ram") + get("rtc_reserved"),
    )
    ...
    for i in range(0, len(raw), 32):
        ...
        if entry[:2] == b"\xeb\xeb":
            continue
        magic, part_type, _subtype, _offset, size, _name, _flags = (
            struct.unpack("<2sBBLL16sL", entry)
        )
        if magic != b"\xaa\x50":
            raise SystemExit("invalid partition table entry")
        if part_type == 0:
            app_sizes.append(size)
    flash = (app_bin.stat().st_size, min(app_sizes))

    def pct(used, total):
        return f"{used * 100.0 / total:.2f}%"

    print()
    print(f"ESP RAM Usage: {dram[0]} bytes / {dram[1]} bytes {pct(*dram)}")
    print(
        f"ESP Flash-App Usage: {flash[0]} bytes / {flash[1]} bytes "
        f"{pct(*flash)}"
    )
